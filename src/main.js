  ...
  if (process.env.DASHBOARD_CAPTURE_PATH) {
    ...
    controlWin.webContents.once('did-finish-load', () => setTimeout(async () => {
      ...
      if (process.env.TOWN_FEATURE_CAPTURE_PATH) {
        const featureReport=await controlWin.webContents.executeJavaScript(`(async()=>{document.querySelector('#townButton').click();await new Promise(r=>setTimeout(r,500));window.TownApp.enterPlace('小菜园');const garden={title:document.querySelector('#townContextTitle').textContent,progress:document.querySelector('.town-growth b')?.textContent};window.TownApp.leavePlace();townState.seeds=30;window.TownApp.enterPlace('鼠鼠小屋');const supply=[...document.querySelectorAll('.town-furniture-label')].find(x=>x.textContent.includes('点击补给')),foodBefore=townState.food;supply?.click();const supplyWorked=townState.food===foodBefore+5;window.TownApp.leavePlace();window.TownApp.focusResident(0);await new Promise(r=>setTimeout(r,100));const state={panel:document.body.dataset.currentPanel,contextVisible:!document.querySelector('#townContext').hidden,contextTitle:document.querySelector('#townContextTitle').textContent,focusedResident:window.TownApp.inspect().focusedResident,actions:document.querySelectorAll('#townContext [data-town-action]').length,statusRows:getComputedStyle(document.querySelector('.town-status')).gridTemplateRows,status:[...document.querySelectorAll('.town-status b')].map(x=>x.textContent),garden,supplyLabel:!!supply,supplyWorked};return state})()`);
