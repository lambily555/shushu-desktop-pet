  ...
  if (process.env.DASHBOARD_CAPTURE_PATH) {
    ...
    controlWin.webContents.once('did-finish-load', () => setTimeout(async () => {
      ...
      if (process.env.TOWN_FEATURE_CAPTURE_PATH) {
        const featureReport=await controlWin.webContents.executeJavaScript(`(async()=>{document.querySelector('#townButton').click();await new Promise(r=>setTimeout(r,500));window.TownApp.applyWorld({part:'夜晚',weather:{sky:0x91a9a2,light:1},npcs:townState.npcs,alive:townState.alive,pendingFarewell:townState.pendingFarewell});const nightLighting=window.TownApp.inspect();renderTown();window.TownApp.enterPlace('小菜园');const garden={title:document.querySelector('#townContextTitle').textContent,progress:document.querySelector('.town-growth b')?.textContent};window.TownApp.leavePlace();townState.seeds=30;window.TownApp.enterPlace('鼠鼠小屋');const supply=[...document.querySelectorAll('.town-furniture-label')].find(x=>x.textContent.includes('点击补给')),foodBefore=townState.food;supply?.click();const supplyWorked=townState.food===foodBefore+5,indoor=window.TownApp.inspect();window.TownApp.focusPet();await new Promise(r=>setTimeout(r,60));const mainInfoTitle=document.querySelector('#townContextTitle').textContent;window.TownApp.clearFocus();window.TownApp.focusResident(5);await new Promise(r=>setTimeout(r,60));const indoorNpcTitle=document.querySelector('#townContextTitle').textContent;window.TownApp.clearFocus();window.TownApp.leavePlace();townState.social={date:'',total:0,byNpc:{}};townState.stamina=100;window.TownApp.focusResident(0);await new Promise(r=>setTimeout(r,100));document.querySelector('[data-town-action="social"]')?.click();document.querySelector('[data-town-action="social"]')?.click();const socialButton=document.querySelector('[data-town-action="social"]'),inspection=window.TownApp.inspect(),state={panel:document.body.dataset.currentPanel,contextVisible:!document.querySelector('#townContext').hidden,contextTitle:document.querySelector('#townContextTitle').textContent,focusedResident:inspection.focusedResident,gaitBoneCount:inspection.gaitBoneCount,armTucked:inspection.armTucked,forepawSpan:inspection.forepawSpan,gaitSample:inspection.gaitSample,nightLighting:{streetLampCount:nightLighting.streetLampCount,litStreetLampCount:nightLighting.litStreetLampCount},indoor:{petVisible:indoor.petVisible,visibleResidentCount:indoor.visibleResidentCount,mainInfoTitle,indoorNpcTitle},actions:document.querySelectorAll('#townContext [data-town-action]').length,statusRows:getComputedStyle(document.querySelector('.town-status')).gridTemplateRows,status:[...document.querySelectorAll('.town-status b')].map(x=>x.textContent),garden,supplyLabel:!!supply,supplyWorked,socialDisabled:socialButton?.disabled,socialDetail:socialButton?.querySelector('span')?.textContent};return state})()`);
      ...
      if (process.env.DASHBOARD_TEST_REPORT) {
        const report = await controlWin.webContents.executeJavaScript(`(async()=>{
          ...
          return {results,returnedHome,profileOpened,profilePushesContent,profileClosed,settingsPageOpened,feedbackPageOpened,emptyFeedbackBlocked,filledFeedbackEnabled,centeredActions,effectControls,pawDisabledStopsTrail,clickDisabledStopsEffect,englishApplied,spanishApplied,englishCornerButtonsClear,spanishCornerButtonsClear,labelsInitiallyClear,backgroundActuallyMoves,visualEffects};
        })()`);
...
function localHamsterReply(message){
  ...
  const englishWords=['hi','hello','hey','thanks','goodbye','morning','afternoon','evening','how','what','when','where','food','hungry','wheel','birthday','sad','tired','work','study','home','favorite'];
  const languageScore=words=>words.reduce((score,word)=>score+(new RegExp(`\\b${word}\\b`,'i').test(text)?1:0),0),spanishScore=languageScore(spanishWords),englishScore=languageScore(englishWords);
  const lang=/[\u3400-\u9fff]/.test(message)?'zh':spanishScore>englishScore?'es':englishScore>spanishScore?'en':(settings.interfaceLanguage||'zh');
  ...
  if(lang==='en'){
    ...
    if(has('where do you live','your home','your house','wooden house'))return 'I live in my warm little wooden house. It is where I rest, hide snacks, and feel safest.';
    if(has('tired','exhausted','bad day','hard day','comfort me','cheer me up','sad today','feel lonely','lonely'))return 'Come stay beside me for a moment. You do not have to solve everything right now; breathe slowly, and I will quietly keep you company.';
    if(has('work','working','study','studying','homework','exam','focus'))return 'Take it one small step at a time. I can sit here with you while you work, and we can take a short break when you finish this part.';
    if(has('bored','nothing to do'))return 'We could chat, you could pet me, or I could run on my wheel for you. Which sounds best?';
    if(has('pet you','touch you','can i pet','hug you'))return 'Yes, gently please. I especially like a soft touch on my head and cheeks.';
  ...
  if(lang==='es'){
    ...
    if(has('donde vives','tu casa','tu hogar','casita de madera'))return 'Vivo en mi casita de madera, calentita y segura. Allí descanso y escondo alguna merienda.';
    if(has('cansado','cansada','agotado','agotada','mal dia','dia dificil','consuelame','animame','triste hoy','solo','sola'))return 'Quédate conmigo un momento. No tienes que resolverlo todo ahora; respira despacio y yo te acompañaré tranquilamente.';
    if(has('trabajo','trabajando','estudio','estudiando','tarea','examen','concentrarme'))return 'Haz una pequeña parte cada vez. Me quedaré aquí mientras trabajas y descansaremos un poco al terminar esta parte.';
    if(has('aburrido','aburrida','nada que hacer'))return 'Podemos hablar, puedes acariciarme o puedo correr en la rueda. ¿Qué prefieres?';
    if(has('acariciarte','tocarte','puedo acariciar','abrazarte'))return 'Sí, pero con suavidad. Me gustan especialmente las caricias en la cabeza y las mejillas.';
  ...
  if(has('住哪里','住在哪','你的家','小木屋'))return '我住在暖暖的小木屋里，那里可以睡觉、藏一点零食，也让我觉得很安心。';
  if(has('累了','好累','疲惫','难受','今天很糟','不开心','安慰我','鼓励我','孤单','寂寞'))return '来我旁边待一会儿吧。现在不用急着把所有事情解决，慢慢呼吸，我会安静陪着你。';
  if(has('工作','上班','学习','作业','考试','专注'))return '先完成眼前的一小步就好。我会在这里陪你，做完这一段我们再休息一下。';
  if(has('无聊','没事做','不知道干嘛'))return '可以和我聊聊天、摸摸我，或者让我跑一会儿跑轮。你想选哪一个？';
  if(has('摸你','抱你','碰你','可以摸'))return '可以呀，动作轻一点。我最喜欢被轻轻摸摸脑袋和脸颊。';
