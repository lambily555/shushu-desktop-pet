...
function init() {
  ...
  function applyWorld(next={}){
    worldState=next;const night=['夜晚','深夜'].includes(next.part),dusk=next.part==='傍晚',lampsOn=night||dusk,sky=new THREE.Color(next.weather?.sky||0xcbd7a9);if(night)sky.multiplyScalar(.12);else if(dusk)sky.multiplyScalar(.55);scene.background.copy(sky);scene.fog.color.copy(sky);ambient.intensity=(night?.35:dusk?1.1:2.4)*(next.weather?.light||1);sun.intensity=(night?.22:dusk?1.15:3.1)*(next.weather?.light||1);sun.color.set(night?0x7995c2:dusk?0xffb66f:0xffe4bd);lampBulbs.forEach(({material,light,pool})=>{material.emissiveIntensity=lampsOn?(night?1.8:1.6):0;light.intensity=lampsOn?(night?7:7):0;pool.material.opacity=lampsOn?(night?.04:.07):0});document.body.dataset.townPart=next.part||'';
