...
function hamster(tone=0x9b9d98) {
  ...
  hamsterAsset.then(asset=>{
    ...
    const tuckShoulder=(armName,foreArmName)=>{const arm=bones[armName],foreArm=bones[foreArmName];if(!arm||!foreArm||!arm.bone.parent)return;const shoulder=model.worldToLocal(arm.bone.getWorldPosition(new THREE.Vector3())),chest=new THREE.Vector3(shoulder.x*.72,shoulder.y-size.y*.16,shoulder.z+size.z*.035),target=arm.bone.parent.worldToLocal(model.localToWorld(chest)),current=foreArm.bone.position.clone().normalize().applyQuaternion(arm.base),desired=target.sub(arm.bone.position).normalize(),correction=new THREE.Quaternion().setFromUnitVectors(current,desired);arm.base=correction.multiply(arm.base.clone());arm.bone.quaternion.copy(arm.base)};
    tuckShoulder('mixamorig:LeftArm','mixamorig:LeftForeArm');tuckShoulder('mixamorig:RightArm','mixamorig:RightForeArm');model.updateMatrixWorld(true);
    const tuckForearm=(foreArmName,handName)=>{const foreArm=bones[foreArmName],hand=bones[handName];if(!foreArm||!hand||!foreArm.bone.parent)return;const elbow=model.worldToLocal(foreArm.bone.getWorldPosition(new THREE.Vector3())),side=Math.sign(elbow.x)||(foreArmName.includes('Left')?1:-1),paw=new THREE.Vector3(side*size.x*.105,center.y+size.y*.035,box.max.z+size.z*.035),target=foreArm.bone.parent.worldToLocal(model.localToWorld(paw)),current=hand.bone.position.clone().normalize().applyQuaternion(foreArm.base),desired=target.sub(foreArm.bone.position).normalize(),correction=new THREE.Quaternion().setFromUnitVectors(current,desired);foreArm.base=correction.multiply(foreArm.base.clone());foreArm.bone.quaternion.copy(foreArm.base)};
    tuckForearm('mixamorig:LeftForeArm','mixamorig:LeftHand');tuckForearm('mixamorig:RightForeArm','mixamorig:RightHand');model.updateMatrixWorld(true);
    const leftHand=bones['mixamorig:LeftHand']?.bone,rightHand=bones['mixamorig:RightHand']?.bone,forepawSpan=leftHand&&rightHand?leftHand.getWorldPosition(new THREE.Vector3()).distanceTo(rightHand.getWorldPosition(new THREE.Vector3())):null;
    rig.userData.joints=joints;rig.userData.bones=bones;rig.userData.model=model;rig.userData.modelBase=model.rotation.clone();rig.userData.armTucked=true;rig.userData.forepawSpan=forepawSpan;rig.userData.loaded=true;
...
function init() {
  ...
  const streetLights=new THREE.Group(),lampBulbs=[];
  function streetLamp(x,z){const lamp=new THREE.Group(),metal=new THREE.MeshStandardMaterial({color:0x4c554b,roughness:.72}),shade=new THREE.MeshStandardMaterial({color:0x6c735f,roughness:.7}),bulbMaterial=new THREE.MeshStandardMaterial({color:0xffe3a4,emissive:0xffc766,emissiveIntensity:0});const pole=new THREE.Mesh(new THREE.CylinderGeometry(.045,.065,1.65,10),metal);pole.position.y=.83;const arm=new THREE.Mesh(new THREE.BoxGeometry(.48,.055,.055),metal);arm.position.set(.2,1.61,0);const cap=new THREE.Mesh(new THREE.ConeGeometry(.22,.2,16),shade);cap.position.set(.41,1.49,0);cap.rotation.z=Math.PI;const bulb=new THREE.Mesh(new THREE.SphereGeometry(.105,14,10),bulbMaterial);bulb.position.set(.41,1.42,0);const light=new THREE.PointLight(0xffc978,0,4.8,2);light.position.copy(bulb.position);lamp.add(pole,arm,cap,bulb,light);lamp.position.set(x,0,z);lamp.rotation.y=(x+z)%2?Math.PI:0;streetLights.add(lamp);lampBulbs.push({material:bulbMaterial,light})}
  [[-4,-.75],[0,-.75],[4,-.75],[-.8,-4],[-.8,3.8],[.8,-2.2],[.8,5.2],[-5.2,2.25],[5.2,2.25]].forEach(([x,z])=>streetLamp(x,z));scene.add(streetLights);
  ...
  function enterPlace(name){
    ...
    if(indoor){buildRoom(name);scene.children.forEach(child=>{if(!child.isLight&&child!==room)child.visible=false});room.visible=true;pet.visible=worldState.alive!==false||worldState.pendingFarewell?.phase!=='buried';const residentIndex=places.findIndex(place=>place[0]===name),resident=residents[residentIndex];if(resident)resident.rig.visible=worldState.npcs?.[residentIndex]?.alive!==false;target.set(0,0,0);distance=13;pitch=.92;yaw=.18}
  ...
  function leavePlace(){
    if(!activePlace)return;room.visible=false;clearRoom();scene.children.forEach(child=>{if(child!==room)child.visible=true});yaw=savedCamera.yaw;pitch=savedCamera.pitch;distance=savedCamera.distance;target.copy(savedCamera.target);activePlace=null;savedCamera=null;returnButton.hidden=true;delete host.dataset.place;applyWorld(worldState);positionCamera();
  }
  function focusResident(index){
    const npc=residents[index];if(!npc?.rig.visible)return;if(focusedResident===-1)savedFocusCamera={yaw,pitch,distance,target:target.clone()};focusedResident=index;target.copy(npc.rig.position).add(new THREE.Vector3(1.05,.25,0));yaw=.15;pitch=.68;distance=5.4;returnButton.hidden=false;host.dataset.resident=String(index);document.querySelector('#townPlace b').textContent=worldState.npcs?.[index]?.name||roles[index];document.querySelector('#townPlace span').textContent=places[index][0]+'的居民';positionCamera();sayToResident(index,'今天也很高兴见到你！');window.dispatchEvent(new CustomEvent('town-npc-select',{detail:{index}}));
  }
  function focusPet(){if(!pet.visible)return;if(focusedResident===-1)savedFocusCamera={yaw,pitch,distance,target:target.clone()};focusedResident=-2;target.copy(pet.position).add(new THREE.Vector3(1.05,.25,0));yaw=.15;pitch=.68;distance=5.4;returnButton.hidden=false;host.dataset.resident='main';speech.hidden=true;document.querySelector('#townPlace b').textContent='我的鼠鼠';document.querySelector('#townPlace span').textContent='你正在陪伴的桌面伙伴';positionCamera();window.dispatchEvent(new CustomEvent('town-main-select'))}
  function clearFocus(restore=true){if(focusedResident===-1)return;focusedResident=-1;speech.hidden=true;delete host.dataset.resident;document.querySelector('#townPlace b').textContent=activePlace||'中心广场';document.querySelector('#townPlace span').textContent=activePlace?'正在查看这里的鼠鼠和设施':'鼠鼠们碰面和交换消息的地方';if(restore&&savedFocusCamera){yaw=savedFocusCamera.yaw;pitch=savedFocusCamera.pitch;distance=savedFocusCamera.distance;target.copy(savedFocusCamera.target);positionCamera()}savedFocusCamera=null;if(!activePlace)returnButton.hidden=true}
  ...
  const clock=new THREE.Clock();function draw(){requestAnimationFrame(draw);const t=clock.getElapsedTime(),inside=indoorNames.includes(activePlace),home=inside?{x:-1.15,z:.85}:(pet.userData.home||{x:2.3,z:2.3});walk(pet,t,0,home.x,home.z);weatherFx.rotation.y=t*.025;if(weatherFx.children[0])weatherFx.children[0].position.y=-(t*2)%4;const occupied=[];labels.forEach(({button,point})=>placeLabel(button,point,occupied,!!activePlace||focusedResident!==-1));residents.forEach((npc,index)=>{const isIndoorResident=inside&&places[index][0]===activePlace;walk(npc.rig,t,npc.phase,isIndoorResident?1.15:npc.x,isIndoorResident?.55:npc.z);placeLabel(npc.tag,npc.rig.position.clone().add(new THREE.Vector3(0,.8,0)),occupied,!npc.rig.visible||!!activePlace||(focusedResident!==-1&&focusedResident!==index))});roomLabels.forEach(({button,point})=>placeLabel(button,point,occupied,false));if(focusedResident>=0&&!speech.hidden){const p=residents[focusedResident].rig.position.clone().add(new THREE.Vector3(0,1.05,0)).project(camera);speech.style.left=((p.x+1)*host.clientWidth/2)+'px';speech.style.top=((-p.y+1)*host.clientHeight/2)+'px'}if(document.body.dataset.currentPanel==='town')renderer.render(scene,camera)}draw();
  function applyWorld(next={}){
    worldState=next;const night=['夜晚','深夜'].includes(next.part),dusk=next.part==='傍晚',lampsOn=night||dusk,sky=new THREE.Color(next.weather?.sky||0xcbd7a9);if(night)sky.multiplyScalar(.38);else if(dusk)sky.multiplyScalar(.72);scene.background.copy(sky);scene.fog.color.copy(sky);ambient.intensity=(night?.9:dusk?1.65:2.4)*(next.weather?.light||1);sun.intensity=(night?.72:dusk?1.8:3.1)*(next.weather?.light||1);sun.color.set(night?0xaec6ef:dusk?0xffc58b:0xffe4bd);lampBulbs.forEach(({material,light})=>{material.emissiveIntensity=lampsOn?(night?2.8:1.7):0;light.intensity=lampsOn?(night?12:7):0});document.body.dataset.townPart=next.part||'';
    pet.visible=next.alive!==false||next.pendingFarewell?.phase!=='buried';residents.forEach((resident,i)=>{const data=next.npcs?.[i],inside=indoorNames.includes(activePlace);resident.rig.visible=data?.alive!==false&&(!inside||places[i][0]===activePlace);resident.tag.hidden=!resident.rig.visible;resident.tag.textContent=data?`${data.name} ${data.sex==='male'?'♂':'♀'}`:roles[i]});
  ...
  window.TownApp={resize,enterPlace,leavePlace,focusResident,focusPet,clearFocus,sayToResident,applyWorld,inspect:()=>({activePlace,focusedResident,interiorVisible:room.visible,petVisible:pet.visible,visibleResidentCount:residents.filter(n=>n.rig.visible).length,streetLampCount:lampBulbs.length,litStreetLampCount:lampBulbs.filter(item=>item.light.intensity>0).length,furniture:roomLabels.map(x=>x.button.textContent),camera:{yaw,pitch,distance,target:target.toArray()},npcCount:residents.filter(n=>n.rig.userData.loaded).length,petLoaded:!!pet.userData.loaded,jointCount:pet.userData.joints?.length||0,gaitBoneCount:Object.keys(pet.userData.bones||{}).filter(name=>/Arm|Leg|Hand|Foot|Spine|Neck|Head/.test(name)).length,armTucked:pet.userData.armTucked,forepawSpan:pet.userData.forepawSpan,gaitSample:pet.userData.gaitSample,petHeight:new THREE.Box3().setFromObject(pet).getSize(new THREE.Vector3()).y,positions:residents.map(n=>n.rig.position.toArray())})};
}
