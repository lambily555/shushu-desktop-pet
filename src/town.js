...
function init() {
  ...
  let yaw=0,pitch=.83,distance=25,drag=null,activePlace=null,savedCamera=null;
  ...
  function buildRoom(name){
    ...
    if(name==='鼠鼠小屋'){
      ...
      shelf(.2,-2.4,'粮仓');add(1,.55,.75,-2.3,.3,1.1,0x9b7851);label('木桌',-2.3,1.1);add(.65,.15,.65,-1.3,.1,1.3,0x92a082);label('坐垫',-1.3,1.3);
  ...
  const clock=new THREE.Clock();function draw(){requestAnimationFrame(draw);const t=clock.getElapsedTime();walk(pet,t,0,2.3,2.3);residents.forEach(npc=>{walk(npc.rig,t,npc.phase,npc.x,npc.z);const p=npc.rig.position.clone().add(new THREE.Vector3(0,.8,0)).project(camera);npc.tag.style.left=((p.x+1)*host.clientWidth/2)+'px';npc.tag.style.top=((-p.y+1)*host.clientHeight/2)+'px';npc.tag.hidden=p.z>1||!!activePlace});labels.forEach(({button,point})=>{const p=point.clone().project(camera);button.style.left=((p.x+1)*host.clientWidth/2)+'px';button.style.top=((-p.y+1)*host.clientHeight/2)+'px';button.hidden=p.z>1||!!activePlace});roomLabels.forEach(({button,point})=>{const p=point.clone().project(camera);button.style.left=((p.x+1)*host.clientWidth/2)+'px';button.style.top=((-p.y+1)*host.clientHeight/2)+'px'});if(document.body.dataset.currentPanel==='town')renderer.render(scene,camera)}draw();
